...
namespace GMap.NET.MapProviders
{
    ...
    public class GMapProviders
    {
        static GMapProviders()
        {
            // <----------
            #region Comment
            Debug.WriteLine("GMapProviders: start !!");
            #endregion
            ...
            Debug.WriteLine(
                string.Concat(
                    "GMapProviders: Plungin Number : ",
                    Convert.ToString(strArray.Length)
                )
            );
            foreach (String str in strArray)
            {
                Debug.WriteLine(
                    string.Concat(
                        "GMapProviders: Plugin File Name : ",
                        str
                    )
                );
                i++;
            ...
            if (File.Exists(strUrlCounterFileName))
            {
                // Url Coutnerファイルが存在する。
                using (StreamReader reader = new StreamReader(@strUrlCounterFileName, sjisEnc))
                {
                    GMapProvider.strUrlCountDate = reader.ReadLine();
                    while ((strLine = reader.ReadLine()) != null)
                    {
                        string[] arrData = strLine.Split(',');
                        GMapProvider.dicUrlCount[arrData[0]] = int.Parse(arrData[1]);
                    }
                }
            }
            else
            {
                // Url Coutnerファイルが存在しない。
                // 現在日付取得
                DateTime dtNow = DateTime.Now;
                GMapProvider.strUrlCountDate = dtNow.ToString("yyyy/MM/dd");
            }
        ...
        // 地図プラグイン用MapProvider
        public static DllMapProvider DllMapDll = DllMapProvider.Instance;
      ...
      public static readonly OpenStreetMapSurferTerrainProvider OpenStreetMapSurferTerrain = OpenStreetMapSurferTerrainProvider.Instance;
        ...
        public static readonly AMapSateliteProvider GaoDeSatellite = AMapSateliteProvider.Instance;
        ...
        public static Dictionary<int, GMapProvider> DbHash;
    ...
    public abstract class GMapProvider
    {
        ...
        // URLに対するアクセス回数保存用Dictionary
        public static Dictionary<String, int> dicUrlCount = new Dictionary<String, int>();
        public static String strUrlCountDate;
