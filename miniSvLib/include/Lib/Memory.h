...
namespace Memory
{
	...
	///
	/// @brief  RAM具象クラス（SDRAM）
	///
	///         新システムでビルドする場合に使用する
	///
	class Sdram : public IRam
	{
		static bool   s_InitializeSucceeded;
	...
	public:
		...
		{
			return m_memory_block.GetAllocateSize(m_length);
		}
	};
	...
	public:
		...
		Element()
		{
			m_ptr = m_memory_block.Allocate(1);
		};

		///
		/// @brief  アロー演算子のサポート
		///
		///         アロー演算子の指定によりオブジェクトをポインタとしてアクセスできるようにします
		///
		...
		inline size_t GetAllocatedMemorySize() const
		{
			return m_memory_block.GetAllocateSize(1);
		}
	};
