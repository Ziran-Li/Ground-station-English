...
typedef enum 
{
	SYSFLAG_NULL                   = 0x00000000, //!< フラグなし
	SYSFLAG_PROPODATAAVAILABLE     = 0x00000001, //!< プロポデータ受信済
	SYSFLAG_THROTTLEBOTTOM         = 0x00000002, //!< プロポのスロットルがローのポジションにある
	SYSFLAG_MCUCOMMTRANSMITENABLE  = 0x00000004, //!< 下位制御MCUから上位制御MCUへのデータ送出を行う
	SYSFLAG_HIGHERMCUDATAAVAILABLE = 0x00000008, //!< 上位制御MCUからの制御入力データ受信済
	SYSFLAG_HIGHERMCUDATAUSING     = 0x00000010, //!< 上位制御MCUからの制御入力データを使用中
	SYSFLAG_DRIVELOCKENABLED       = 0x00000020, //!< ロータ始動禁止状態（一度セットされると解除されない）
} SYSFLAG;
